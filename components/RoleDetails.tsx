        ...
        <div className="flex flex-col md:flex-row gap-12 items-start">
          ...
          <div className="md:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-6">
            {tasks.map((task, index) => (
              <Card key={index} className={`group hover:border-primary/50 transition-colors duration-300 ${index === tasks.length - 1 ? 'md:col-span-2' : ''}`}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 group-hover:bg-primary/10 transition-colors">
                      {task.icon}
                    </div>
                ...
                <CardContent>
                  <CardTitle className="mb-2 text-white group-hover:text-primary transition-colors">{task.title}</CardTitle>
